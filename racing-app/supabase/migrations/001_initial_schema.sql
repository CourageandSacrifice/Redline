...
CREATE POLICY "Creators can manage collections" ON public.collections FOR ALL USING (
...
CREATE POLICY "Creators can manage clips" ON public.clips FOR ALL USING (
    EXISTS (
...
CREATE POLICY "Creators can manage car info" ON public.car_info FOR ALL USING (
    EXISTS (
...
CREATE POLICY "Creators can manage performance stats" ON public.performance_stats FOR ALL USING (
    EXISTS (
        SELECT 1 FROM public.clips cl
        JOIN public.collections c ON c.id = cl.collection_id
        JOIN public.channels ch ON ch.id = c.channel_id
